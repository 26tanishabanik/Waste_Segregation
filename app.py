...
if option == 'About Model':
  ...
  st.markdown('State-Of-The-Art Models: YOLO is a fast and accurate object detection algorithm which works in real-time and it provides accurate results with minimal background errors. The state-of-the-Art model YOLO v5 was used. This model was custom trained on 5 classes of waste categories and the resulting models have high accuracy and faster inference time.',unsafe_allow_html = True)
  st.markdown('Standard Metrics for Object Detection-Mean Average Precision: mAP score was used to compare and benchmark the performance of our models. Besides mAP, manual review of image inference run on these models was done to perform error analysis. This approach helped in improving the performance of the final model tremendously.',unsafe_allow_html = True)
...
if option == 'About Project':
  ...
  st.text("")
  st.text("")
  st.markdown('The biggest challenge in recycling/re-using waste is sorting and segregating different types of waste since segregation of waste aids in targeted recycling or even decomposition. As an example, segregating a dry metal can from a metal can containing organic matter eases recycling. The necessary action for proper segregation of the waste on a large scale is to identify various materials first.',unsafe_allow_html = True)
  st.markdown('Once identified, neuromorphic tools could be used to sort things based on the identified parameters. However, while there exist several methods to identify different materials such as visual sensors, olfactory sensors as well as spectroscopic tools, there are very few or no attempts at using artificial intelligence to specifically identify materials from the waste, which could then be applied to ease the segregation process.', unsafe_allow_html = True)
  st.markdown('Our current tool based on Computer Vision model enables visual image recognition to first identify objects, in their full form or by parts in order to be used later for segregation. Try it for yourself!', unsafe_allow_html = True)
